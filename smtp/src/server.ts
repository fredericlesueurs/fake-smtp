...
QueueManager.createInstance(
    ...
    (err, queueManager) => {
        if (err) {
            throw new Error('Error on create queue manager instance');
        }

        queueManager?.queue.exists('email', (err, exist) => {
            if (!exist) {
                queueManager?.queue.create('email', false, (err) => {
                    if (err) {
                        throw new Error(`Error on create queue: ${err}`);
                    }
                });
